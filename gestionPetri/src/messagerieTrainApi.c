...
int troncon(int sd, int nTrain, int codeTroncon){
    ...
    DEBUG_PRINT("Troncon : Allumage troncon reussi\n");

    int nTrainRecu=-1, nType=-1, nCapteur=-1;
    CHECK(reponseXway(sd, &nTrainRecu, &nType, &nCapteur), "Reponse Xway fail");

    DEBUG_PRINT("Troncon retour : Train=%d et capteur=%d\n", nTrain, nCapteur);
    DEBUG_PRINT("Troncon nType=%d\n", nType);
...
int aiguillage(int sd, int nTrain, int codeAig){
    ...
    DEBUG_PRINT("Aiguillage : Changement aiguillage reussi\n");
    ...
    DEBUG_PRINT("Aiguillage retour : Train=%d et code requete=%d\n", nTrain, codeRequete);
